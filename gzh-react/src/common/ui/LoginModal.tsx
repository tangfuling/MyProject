...
export default function LoginModal() {
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      setError(ex instanceof Error ? ex.message : '发送验证码失败');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="login-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="login-modal-sub">用手机号登录，新用户自动注册</div>

        <div className="form-field">
          <div className="form-label">手机号</div>
          <input className="input" type="text" placeholder="请输入手机号" value={phone} onChange={(event) => setPhone(event.target.value.trim())} />
        </div>

        <div className="form-field">
          <div className="form-label">验证码</div>
          <div className="code-row">
            <input className="input" type="text" placeholder="请输入验证码" value={code} onChange={(event) => setCode(event.target.value.trim())} />
        ...
        <button className="btn btn-primary" type="button" style={{ width: '100%', padding: '13px', fontSize: '14px', marginTop: '14px' }} disabled={logining} onClick={() => void login()}>
          {logining ? '登录中...' : '登录'}
        </button>
        <div className="login-tip">新用户注册赠送 <b>¥1.00</b> 免费额度</div>
      </div>
    </div>
  );
}
